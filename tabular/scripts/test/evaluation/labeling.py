...
def parse_args():
    parser = argparse.ArgumentParser(description='Test Score Label Performance')
    ...
    parser.add_argument('--use_label', type=int, default=1, help='used label value')
...
def main(args, config, logger):
    set_seed(0)
    # log parameters.
    logger.info(f'====== Test Score Label Performance ======')
    logger.info('\n'.join([f'{k} = {str(v)}' for k, v in sorted(vars(args).items())]))
    
    # set dataset.
    labels   = pd.read_csv(args.score_file)[args.score_col]
    index    = labels[labels==args.use_label].index.tolist()
    trainset = load_cv_dataset(args.dataset, train=True, augment=False)
...
# python -u scripts/test/evaluation/labeling.py --dataset cifar10 --score_file results/evaluations/cifar10/241112-0329/estimation.csv --score_col 'iter:6-pred' --use_label 1 --cuda 0
if __name__ == '__main__':
    args   = parse_args()
    config  = getattr(configs, args.dataset)
    date   = datetime.datetime.now().strftime('%y%m%d-%H%M')
    logger = set_logger(name=f'label-eval-{date}', logfile=args.score_file.replace('.csv', '-eval.log'))
    try:
