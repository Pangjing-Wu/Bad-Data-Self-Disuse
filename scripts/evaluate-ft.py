...
sys.path.append('.')
import configs
from utils.dataset import load_cv_dataset, KFoldDataset, NumpyDataset
...
def parse_args():
    parser = argparse.ArgumentParser(description='Bad Data Self-Disuse (Finetune Version)')
...
def main(args, logger, filepath):
    # log parameters.
    logger.info(f'====== Bad Data Self-Disuse (Finetune Version) ======')
    ...
    # set model.
    if args.embed_net not in ['vit', 'mae', 'beit']:
    ...
    dataloader = DataLoader(dataset=dataset, batch_size=args.bs, shuffle=False)
    with torch.no_grad():
        x = torch.cat([embed(x.to(device)).cpu() for x, _ in dataloader], dim=0).numpy()
        y = np.array(dataset.targets, dtype=int) if hasattr(dataset, 'targets') else torch.cat([y for _, y in dataloader], dim=0).long().numpy()
    ...
    # set finetune model.
    model_fn = partial(nn.Linear, in_features=x.shape[-1], out_features=len(dataset.classes))
    
    # split k folds.
    folds   = KFoldDataset(dataset=dataset, k=args.k, shuffle=True)
    columns = [f'fold-{i}' for i in range(args.k)]
    logger.info(f'split embedded data into {args.k} folds, where each fold contained {len(folds[0])} elements.')
    ...
    for r in range(args.max_iter):
        ...
        with ProcessPoolExecutor(max_workers=args.eval_n_jobs, mp_context=mp.get_context('spawn')) as executor:
            ...
            for i, loss in executor.map(eval_fn, sample_indices):
                ...
                baseline = baselines.values.flatten() if mode == 'all' else baselines.loc[y[i]].values
    ...
    logger.debug(f'baseline preformance under mode {mode}:\n{baselines}')
    logger.debug(f"sample preformance under mode {mode}:\n{pd.DataFrame(np.array(losses[mode]), index=losses['id'], columns=columns)}")
    logger.debug(f'feature weight:\n{pd.DataFrame(np.array(weight), columns=components.columns)}')
    logger.info('done!')


# python -u ./scripts/evaluate-ft.py --dataset cifar10 --collection_date '230827-0851' --k 3 --max_iter 10 --n_eval 50 --eval_n_jobs 3 --embed_net vit --embed_dataset imagenet21k --embed_date '666666-6666' --epoch 5 --bs 128 --cuda 1 --debug
