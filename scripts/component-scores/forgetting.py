...
def parse_args():
    parser = argparse.ArgumentParser(description='Forgetting Event Evaluation')
...
# python -u ./scripts/component-scores/forgetting.py --dataset cifar10 --net resnet18 --epoch 5 --lr 1e-3 --bs 128 --seed 0 1 --cuda 0 --debug
if __name__ == '__main__':
    args    = parse_args()
    date    = datetime.datetime.now().strftime('%y%m%d-%H%M')
    logpath = set_component_log_path(score=SCORE, dataset=args.dataset, model=args.net, date=date)
    filepath = set_component_path(score=SCORE, dataset=args.dataset, model=args.net, date=date, epoch=None)
