...
sys.path.append('.')
import configs
from component_scores import OneClassSVM
...
def parse_args():
    ...
    parser.add_argument('--svm_kernel', type=str, default='rbf', help="`one class svm kernel (default: 'rbf')")
    parser.add_argument('--rbf_gamma', default='scale', help="`max_samples` of isolation forest (default: 'scale')")
...
def main(args, logger, filepath):
    ...
    score = (score - score.mean()) / max(1e-5, score.std())
    ret   = pd.DataFrame(dict(scores=score))
...
# python -u ./scripts/component-scores/one-class-svm.py --dataset cifar10 --embed_net 'vit' --embed_dataset 'imagenet21k' --embed_algo 'pt' --embed_date '666666-6666' --cuda 0 --debug
