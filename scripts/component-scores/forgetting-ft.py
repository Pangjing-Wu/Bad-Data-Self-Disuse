...
SCORE = 'forgetting-event'


def parse_args():
    parser = argparse.ArgumentParser(description='Forgetting Event Evaluation (Finetune Version)')
...
def main(args, logger, filepath):
    # log parameters.
    logger.info(f'====== Calculate Forgetting Event Score (Finetune Version) ======')
    ...
    dataset = load_cv_dataset(args.dataset, train=True, augment=False, resize=param['input_size'], include_index=True)
    ...
    with torch.no_grad():
        i = torch.arange(len(dataset))
        x = torch.cat([embed(x.to(device)).cpu() for _ ,x, _ in dataloader], dim=0)
        y = torch.LongTensor(dataset.targets) if hasattr(dataset, 'targets') else torch.cat([y for _, _, y in dataloader], dim=0)
    dataset = TensorDataset(i, x, y)
...
# python -u ./scripts/component-scores/forgetting-ft.py --dataset cifar10 --embed_net vit --embed_dataset imagenet21k --embed_date '666666-6666' --epoch 5 --lr 1e-2 --bs 128 --seed 0 1 --cuda 0 --debug
