...
sys.path.append('.')
import configs
from component_scores import EL2N
...
def parse_args():
    parser = argparse.ArgumentParser(description='EL2N Evaluation (Finetune Version)')
...
def main(args, logger, filepaths):
    # log parameters.
    logger.info(f'====== Calculate EL2N Score (Finetune Version) ======')
    ...
    for r, seed in enumerate(args.seed):
        ...
        # reset scorer.
        scorer = EL2N(model=model, n_classes=model.out_features, order=args.order)
...
# python -u ./scripts/component-scores/el2n-ft.py --dataset cifar10 --embed_net vit --embed_dataset imagenet21k --embed_date '666666-6666' --epoch 1 2 3 4 5 --lr 1e-2 --bs 128 --seed 0 1 --cuda 0 --debug
