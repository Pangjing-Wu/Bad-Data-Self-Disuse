...
def parse_args():
    parser = argparse.ArgumentParser(description='GraNd Evaluation')
...
def main(args, logger, filepaths):
    ...
    for r, seed in enumerate(args.seed):
        ...
        # reset scorer.
        scorer = GraNd(model=model, n_classes=len(dataset.classes), order=args.order, 
                       optim=torch.optim.Adam, fc_name=args.fc_name)
...
# python -u ./scripts/component-scores/grand.py --dataset cifar10 --net resnet18 --epoch 0 --lr 1e-3 --bs 128 --seed 0 1 --cuda 1 --debug
