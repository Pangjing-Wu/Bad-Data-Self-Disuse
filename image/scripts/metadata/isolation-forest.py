...
sys.path.append('.')
from configs.metadata import iforest as configs
from metadata import MultiClassesIsolationForest
...
def main(args, config, logger, meta_manager):
    ...
    for seed in range(config.repeat):
        set_seed(seed)
        logger.info(f'calculating isolation forest score ({seed+1}/{config.repeat}).')
        scorer = MultiClassesIsolationForest(n_classes=config.n_classes, random_state=seed, **config.iforest_kwargs)
...
if __name__ == '__main__':
    ...
    meta_manager = MetaDataManager('i-forest', dataset=args.dataset, date=date)
    logger  = set_logger(name='i-forest', logfile=meta_manager.logpath)
    try:
