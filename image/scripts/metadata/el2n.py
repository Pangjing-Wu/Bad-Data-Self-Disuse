...
sys.path.append('.')
from configs.metadata import el2n as configs
from metadata import EL2N
...
def parse_args():
    parser = argparse.ArgumentParser(description='EL2N')
...
def main(args, config, logger, meta_manager):
    # log parameters.
    logger.info(f'====== Calculate EL2N Score ======')
    ...
    for seed in range(config.repeat):
        set_seed(seed)
        logger.info(f'calculating EL2N score ({seed+1}/{config.repeat}).')
        # reset data loader.
        trainloader = DataLoader(dataset=dataset, batch_size=config.bs, shuffle=True, num_workers=0)
        # reset model.
        model     = model_cls(n_classes=config.n_classes, **config.model_kwargs).to(device)
        ...
        # reset scorer.
        scorer = EL2N(model=model, n_classes=config.n_classes, order=config.order)
...
if __name__ == '__main__':
    ...
    meta_manager = MetaDataManager('el2n', dataset=args.dataset, date=date)
    logger = set_logger(name='el2n', logfile=meta_manager.logpath)
    try:
